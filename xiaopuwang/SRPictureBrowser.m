...
- (void)setup {
    ...
    [self addSubview:({
        _indexLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 20, screenBounds.size.width, 21)];
        _indexLabel.textColor = [UIColor whiteColor];
        _indexLabel.textAlignment = NSTextAlignmentCenter;
        _indexLabel.font = [UIFont systemFontOfSize:17.0];
        _indexLabel.text = [NSString stringWithFormat:@"%ld/%lu",(long)self.currentIndex+1,(unsigned long)self.pictureModels.count];
        _indexLabel;
    })];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    self.indexLabel.text = [NSString stringWithFormat:@"%ld/%lu",(long)self.currentIndex+1,(unsigned long)self.pictureModels.count];
